...
teste_rental = remove_outlier(teste_rental, 'pricingInfos.rentalTotalPrice',10)
...
le = LabelEncoder()
le.fit(teste_rental['unitTypes'])
teste_rental['unitTypes'] = le.transform(teste_rental['unitTypes'])
...
x_treino = data_3_rental[['address.geoLocation.location.lat','address.geoLocation.location.lon','bathrooms'
                         ,'bedrooms','parkingSpaces','pricingInfos.yearlyIptu','suites','unitTypes','usableAreas']]
y_treino = data_3_rental['pricingInfos.rentalTotalPrice']


x_teste = teste_rental[['address.geoLocation.location.lat','address.geoLocation.location.lon','bathrooms'
                         ,'bedrooms','parkingSpaces','pricingInfos.yearlyIptu','suites','unitTypes','usableAreas']]
y_teste = teste_rental['pricingInfos.rentalTotalPrice']
...
final_csv = pd.DataFrame({"Valor Real": teste_rental["pricingInfos.rentalTotalPrice"],
                          "Valor Predict":y_pred })

final_csv.to_csv(r'C:\Users\Lucas Reis\Desktop\Lucas\DataScience\Prever com RNF rental k=6.csv', index=False)